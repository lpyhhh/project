...
def draw_pie(profix_file, out_prefix):
    ...
    (
    Pie(init_opts=opts.InitOpts(width="1600px", height="900px"))
    .add(
	center = ['70%', '50%'],
        series_name="Level1",
...
def main():
    ...
    VFDB_level_table = os.path.join("/project/RD/Metagenome/database/VF/VFDB_level_gene_link.txt")
    ...
    with open(VFDB_level_table) as fh:
        for line in fh:
            ...
            if len(fields) <=4:
                continue
            # symbol2level.setdefault(fields[3], set()).add("\t".join(fields[:2]))#{VFs:level1\tlevel2\t-}
            if fields[:2] not in level_list:
                level_list.append(fields[:2])
            ind = level_list.index(fields[:2])
            gene_list = re.split(r',\s*', fields[4])
            for gene in gene_list:#gene VFG***
                if gene != "" or gene != '-':
                    VFG2index[gene] = ind #VFs bscU...
    ...
    with open(sys.argv[1]) as fh:
        for line in fh:
            ...
            fields = line.strip().split("\t")
            vfid = fields[2] #VFG***
            vf_symbol = fields[11] # VFs
            if vfid in VFG2index:
                level = "\t".join(level_list[VFG2index[vfid]])
