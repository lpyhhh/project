...
"""
python3 scripts/run_genomad.py \
    --results-megahit /media/jjz/hdd/lpy/meige/SCUT-zou/results/megahit \
    --results-genomad /media/jjz/hdd/lpy/meige/SCUT-zou/results/genomad \
    --threads 32 \
    --db /media/jjz/hdd/lpy/DB/genomad/genomad_db \
    --log-dir /media/jjz/hdd/lpy/meige/SCUT-zou/logs/genomad
"""

def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Wrapper to run genomad and print sample names")
...
def main() -> None:
    ...
    cmd: List[str] = [
        "python3",
        str(ROOT / "steps" / "genomad.py"),
        "--results-megahit",
        str(results_megahit),
        "--results-genomad",
        args.results_genomad,
        "--threads",
        str(args.threads),
        "--db",
        args.db,
